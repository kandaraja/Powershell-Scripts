...
If ($Region -eq "US")
{
Net user US-QGuard Welcome@ABB123 /ADD /FULLNAME:"US-QGuard"
cmd.exe /C "WMIC USERACCOUNT WHERE Name='US-QGuard' SET PasswordExpires=FALSE"
Net localgroup Administrators US-QGuard /add
}
...
netdom join $Comp /domain:abb.com /userd:europe\xe-admin-pm3 /passwordd:*
 }
  else {
    If ($LIP -eq $TIP) {
    ...
    Write-Host "No action performed, Run this Script again after reboot"  -ForegroundColor Yellow
    shutdown /r /t 020
     }
       }
         }
